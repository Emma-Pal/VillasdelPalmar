...
router.post('/panel/usuarios/nuevo', requireMesa, (req, res) => {
  const { tipo, nombre, unidad, cargo, usuario, password } = req.body;

  if (!password) {
    return renderForm(res, { error: 'La contraseña es obligatoria al crear una cuenta.' });
  }
  ...
  try {
    crearUsuario({
      tipo,
      nombre,
      unidad: tipo === 'propietario' ? unidad : null,
...
router.post('/panel/usuarios/:id/editar', requireMesa, (req, res) => {
  ...
  try {
    ...
    if (req.session.user.id === id) {
      req.session.user = { id, tipo, nombre, unidad: unidad || null, cargo: cargo || null };
    }
...
router.post('/panel/usuarios/:id/eliminar', requireMesa, (req, res) => {
  const id = Number(req.params.id);
